...
class App {
    ...
    async getData(initialRender) {
        ...
        departments.forEach(department => {
            const location = Location.getByName(department.location);
            new Department({ ...department, location })
        })

        employees.forEach(employee => {
            const location = Location.getByName(employee.location);
            const department = Department.getByName(employee.department);
    ...
    insertFormOptions() {
        ...
        if (department && location) {
            this._departments.list.forEach(dept => createOption(dept, department));
            this._locations.list.forEach(loc => createOption(loc, location));
        } else if (deptLocation) {
            this._locations.list.forEach(loc => createOption(loc, deptLocation));
        }
    }
        ...
        ids.forEach(id => {
            ...
            if (id.includes('add')) {
                buttons[id].addEventListener('click', e => {
                    ...
                    this._formModal.handleModalShow(e)
                })
            } else {
    ...
    async willRun() {
        ...
        document.getElementById('logo').addEventListener('click', e => window.scrollTo({
